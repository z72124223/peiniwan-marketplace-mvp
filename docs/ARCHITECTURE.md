...
`/live` 是私人 Demo 的雙角色操作台。`provider_shifts` 保存陪玩師班次，`provider_invitations` 保存玩家、陪玩師、服務、保留點數、60 秒期限與最終結果；`/api/live-marketplace` 將打卡、下班、邀請、接受、拒絕與逾時動作集中在伺服器端。

邀請建立時以 D1 批次操作再次確認陪玩師仍在線、玩家餘額足夠，並同時將玩家點數由可用轉為保留。接受只會建立一張示範訂單；拒絕或逾時只會返點一次、結束班次並把陪玩師移出在線池。唯一索引限制同一陪玩師與 Demo 玩家同時只能有一筆待處理邀請。

目前沒有正式登入、通知與背景排程器。頁面倒數或後續 `GET` 狀態讀取會呼叫冪等逾時處理；若要進入正式營運，必須另加受信任的伺服器排程與角色授權。陪玩師逾時扣點金額尚未核定，因此 v0.6 不會自行執行這項金錢動作。
